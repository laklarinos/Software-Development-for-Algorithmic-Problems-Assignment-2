...
hashTableCube::hashTableCube(int size, cubeConstants *cubeCon, int numOfDimensions)
{
    this->array.resize(size);
    this->size = size;
    this->cubeCon = cubeCon;
    this->rVector.resize(cubeCon->k);
    ...
    for (int i = 0; i < this->size; i++)
    {
        decToBin(&this->zeroAndOneVect[i], this->cubeCon->k, i);
    }
    ...
    this->vAndTVector.resize(cubeCon->k);
    int sample;

    for (int j = 0; j < cubeCon->k; j++)
    {
        std::uniform_real_distribution<float> uniDis(0, cubeCon->w - 1);
...
int hashTableCube::hashFunction(point *pGiven)
{
    ...
    for (int i = 0; i < cubeCon->k; i++)
    {
        ...
        toLowerBound = vpplust / cubeCon->w;
        hi = toLowerBound;
